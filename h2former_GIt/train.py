...
import H2former.H2former_LR as H2former
...
def train_epoch(model, train_dataset, training_loader, optimizer, opt, epoch):
    ...
    for batch in tqdm(training_loader, mininterval=2,
        ...
        if opt.decoder == 'attention':
            ...
            # batch_y_mark[:, 0:1, :]####[32,1,4]
            batch_x_mark = torch.cat([batch_x_mark, batch_y_mark[:, 0:1, :]], dim=1)####[32,168,4]---[32,169,4]
            outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, False)###[32,168,321]
            # output_test=
        ...
        # compute loss
        f_dim = -1 if opt.features == 'MS' else 0
        outputs = outputs[:, -opt.predict_step:, f_dim:]
        batch_y = batch_y[:, -opt.predict_step:, f_dim:].to(opt.device)
...
def eval_epoch(model, test_dataset, test_loader, opt, epoch):
    ...
    folder_path = './test_results/'+'wea_del'+'/'
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, mininterval=2,
            ...
            if kk % 20 == 0:
                ...
                # del gt_str[168]
                # print(pd_str)
                true_str=','.join(map(str, true[0, :, -1].tolist()))
                pred_str=','.join(map(str, pred[0, :, -1].tolist()))
                ###画图
                # with open("/home/szj/my_code/H2former/huitu_wea.txt", "a+") as f:
                #     f.write('kk: {}'.format(kk))
                #     f.write('\n')
                ...
                # visual(gt, pd, os.path.join(folder_path, str(kk) + '.pdf'))
            kk += 1
...
def train(model, optimizer, scheduler, opt, model_save_dir):
    ...
    for epoch_i in range(opt.epoch):
        ...
        final_metrics = [float(best_mse), float(mae), float(rmse), float(mape), float(mspe)]
...
def evaluate(model, opt, model_save_dir):
    ...
    if best_mse > mse:
        best_mse = mse
        best_metrics = current_metrics
    final_metrics = [float(mse), float(mae), float(rmse), float(mape), float(mspe)]
    # return best_metrics
    return final_metrics
...
def parse_args():
    ...
    parser.add_argument('-model', type=str, default='H2former')
...
def main(opt, iter_index):
    ...
    model_save_dir = 'models/LongRange_new/{}/{}/'.format(opt.data, opt.predict_step)
...
if __name__ == '__main__':
    ...
    with open("/home/szj/my_code/H2former/buchongshiyan.txt", "a+") as f:
        f.write('[Info] parameters: {}'.format(opt))
        f.write('\n')
        f.write('Average Metrics: {}'.format(all_perf))
        f.write('\n')
        f.write('Iteration best metrics: {}'.format(metrics))
        f.write('\n')
        f.close()
    # print('Iteration best metrics: {}'.format(best_metrics))
