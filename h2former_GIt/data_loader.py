...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None, data_path='ETTh1.csv', dataset='ETTh1', inverse=False):
        ...
        self.root_path = root_path####elect数据集data/elect
        self.data_path = data_path####数据名字elericity
        self.__read_data__()
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        num_train = int(len(df_raw)*0.7)####elect train:18412
        num_test = int(len(df_raw)*0.2)#####elect test 5260
        num_vali = len(df_raw) - num_train - num_test####elect_test 2632
        border1s = [0, num_train-self.seq_len, len(df_raw)-num_test-self.seq_len]#####border1s[0,18244,20876]
        border2s = [num_train, num_train+num_vali, len(df_raw)]#####border2s[18412,21044,26304]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]####broder2:18412
...
class Dataset_Custom111(Dataset):
    def __init__(self, root_path, flag='train', size=None, data_path='ETTh1.csv', dataset='elect',
                inverse=False):
        # size [seq_len, label_len, pred_len]
        # info
