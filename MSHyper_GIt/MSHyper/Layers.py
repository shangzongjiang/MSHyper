...
def get_mask(input_size, window_size, inner_size, device):
        ...
        for i in range(start, start + all_size[layer_idx]):
            if (i+1) % 4 == 0 or (i+1 == start + all_size[layer_idx]):
                ...
                if layer_idx==0:
                    ...
                    scale_node = math.ceil((left_side -sum(all_size[:layer_idx]))/ window_size[layer_idx - 1]) + sum(all_size[:layer_idx + 1])
    ...
    concat_ite_ita=np.vstack((np.hstack((mask_intra, np.zeros((mask_intra.shape[0], mask_inter.shape[1])))), np.hstack((np.zeros((mask_inter.shape[0], mask_intra.shape[1])), mask_inter))))
    ...
    # return edge_index, all_size,concat_all
...
class Scale_Construct111(nn.Module):
    """Bottleneck convolution CSCM"""
    def __init__(self, d_model, window_size, d_inner):
    ...
    def forward(self, enc_input):####[32,169,512]
        ...
        # print(all_inputs[1].shape())
        hour_trend = self.hour_trend(all_inputs[0])
        day_trend = self.day_trend(all_inputs[1])
        week_trend = self.week_trend(all_inputs[2])
        hour_trend=self.up(hour_trend.transpose(1, 2))
        day_trend=self.up(day_trend.transpose(1, 2))
        week_trend=self.up(week_trend.transpose(1, 2))
        ...
        return all_inputs,hour_trend,day_trend,week_trend
